...
export default async function FragmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const related = getConstellationFragments(fragment.constellation)
    .filter((item) => item.id !== fragment.id)
    .slice(0, 2);
  const primaryPerson = fragment.people?.[0]
    ? personThreads[fragment.people[0]]
    : null;
  ...
  return (
    <PageChrome
      ...
      actions={
        <Link
          href={`/constellations/${fragment.constellation}` as Route}
        ...
        <div className="space-y-6">
          <div className="glass-panel rounded-[34px] p-6 md:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {fragment.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-white/8 px-3 py-1.5 text-xs text-white/62"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="glass-panel rounded-[34px] p-6">
            ...
            <p className="mt-4 text-base leading-7 text-white/72">
              {fragment.whyResurfaced}
            </p>
            <div className="mt-5 rounded-[26px] border border-white/10 bg-white/[0.03] p-5">
              <p className="text-[0.72rem] uppercase tracking-[0.18em] text-white/44">
                Why this may matter
              </p>
              <p className="mt-3 text-sm leading-7 text-white/72">
                {fragment.whyItMayMatter}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="glass-panel rounded-[34px] p-6">
            ...
            <div className="mt-4 space-y-4 text-sm leading-7 text-white/70">
              ...
              {fragment.place ? (
                <p>
                  <span className="text-white">Place:</span> {fragment.place}
                </p>
              ) : null}
              <p>
                <span className="text-white">Emotional weight:</span>{' '}
                {fragment.emotionalWeight}
              </p>
              {primaryPerson ? (
                <p>
                  <span className="text-white">Relationship thread:</span>{' '}
          ...
          <div className="glass-panel rounded-[34px] p-6">
            ...
            <div className="mt-4 space-y-4">
              <Link
                ...
                className={`block rounded-[24px] bg-[var(--warm)] px-5 py-4 text-sm font-semibold text-slate-950 ${focusRing}`}
              >
                {fragment.nextStepLabel}
              </Link>
              <Link
                href={`/constellations/${fragment.constellation}` as Route}
                className={`block rounded-[24px] border border-white/12 bg-white/5 px-5 py-4 text-left text-sm font-semibold text-white/78 ${focusRing}`}
              >
                Keep this inside the constellation view
              </Link>
              <Link
                href="/reflection"
                className={`block rounded-[24px] border border-white/12 bg-white/5 px-5 py-4 text-left text-sm font-semibold text-white/56 ${focusRing}`}
              >
                Let this rest in weekly reflection
              </Link>
            </div>
          </div>
          ...
          {primaryPerson ? (
            <div className="glass-panel rounded-[34px] p-6">
              ...
              <p className="mt-4 text-sm leading-7 text-white/72">
                {primaryPerson.lowPressurePrompt}
              </p>
              <Link
                href={`/reconnect/${fragment.id}` as Route}
                className={`mt-5 inline-flex text-sm font-semibold text-[var(--warm)] ${focusRing}`}
              >
        ...
        <section className="mt-8 space-y-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white">
              Other fragments in this constellation
            </h2>
          </div>
